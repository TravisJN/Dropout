  ...
  svg.selectAll('.enemy').data(enemies)
     ...
     .tween('Collision Check', function(d, i){  //Run checkCollision at each step in the transition to register collisions while transitioning
       return function(t) {  //t is the percentage of the way through the transition
        ...
        if (collided === false){
        if (checkCollision(d.x, d.y, playerArray[0].x, playerArray[0].y) === true){  
          collided = true;
          score = 0;
          collisions++;
       }
...
var checkCollision = function(targetX, targetY, playerX, playerY){
...
var startGame = function() {
  ...
  d3.select('.gameSpace').selectAll('.player').data(playerArray).on('click', function(d){
      player.y = 493;
      playerDrop = true;
      d3.selectAll('.player').data(playerArray).transition().duration(1000).ease('bounce')
        .attr('cx', player.x).attr('cy', player.y).tween('Collision Check', function(d, i){  //Run checkCollision at each step in the transition to register collisions while transitioning
         return function(t) {  //t is the percentage of the way through the transition
          d.x = d3.select(this).attr('cx');
          d.y = d3.select(this).attr('cy');
          for (var i = 0; i < enemies.length; i++){
            if (checkCollision(enemies[i].x, enemies[i].y, d.x, d.y) === true){  
              playerDrop = false;
            }
